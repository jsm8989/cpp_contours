...
int load_contour(char *contour_filename, struct Contour* cs)/// NOT USING ANYMORE
{
// initiliases (or checks if it already contains info) the cs structure in association with the given filename

	FILE *fptr = fopen(contour_filename,"r"); // NB: changed mode to "w" from "r" to see if that works (should create the file)
	// moved back to read-only as it seems like this file should be "created" at some other point, and the load_contour only gets it into the cs struct

	char c;
	char line_buffer[1000]; // is this a problem if the file ends up having more than 1000 lines?
	...
	// verbose:
	printf("char line_buffer[1000] = %c\n", line_buffer);
	...
	if (fptr == NULL)
	{
		printf("fptr==NULL, terminating load_contour(), ignore following output. \n");
	...
	{ 
		...
		int i = 0;
		while ((c = getc(fptr)) != EOF) // getc gets a character from the stream (pointer to a file) given as argument. Needs a non-empty file...
		{ // for every character in the file that isn't the end of the file

		printf("Current result of getc(fptr): %c\n",c); // for my own understanding - doesn't seem to give anything

			if (c == '\n') 
			{ // if its a blank line, do this (which it should be in the case of creating a new contour file)
				line_buffer[i] = '\0'; // line buffer used in contour_read_mode section. It cuts out the character at line_buffer[i]

				if (strstr(line_buffer, start_tag) != NULL) // if start_tag exists in line_buffer string ie you are at a line containing "<contour_fine>"
				{
					contour_read_mode = 1;
					cf_i = 0;
					if (max_i_found)
				...
				else if (strstr(line_buffer, max_i_tag)) // if at a line containing "max_i = "
				{
					cs->max_i = atoi(line_buffer + strlen(max_i_tag)); // atoi converts string to int
					max_i_found = 1;
				}

				else if (contour_read_mode) // if you have just passed the start tag
				{
					if (strstr(line_buffer, end_tag) != NULL) // ends reading if needed
					{
						contour_read_mode = 0;
					}
					else
					{
						...
						cs->contour[2*cf_i + 1] = atof(p); // atof converts string to double
						*p = '\0';
		...
		printf("If no other output since starting else loop, the file is empty\n");
	}
	return 0;
}
...
//void analyze_contour(double *c, int n)
{
        ...
        {// requires cs to already have some structure
